...
const Resume = () => {
  ...
  const education = [
    { degree: 'Bachelor in Computer Science', institution: 'Anurag Engineering College', year: 'May 2024', gpa: '7.5' },
  ];
        ...
        >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            <motion.div variants={itemVariants}>
              <div className="flex items-center gap-2 mb-6">
                ...
                <h3 className="text-xs font-semibold text-text-muted uppercase tracking-widest">Experience</h3>
              </div>
              <div ref={timelineRef} className="relative pl-5 space-y-8">
                ...
                {experiences.map((exp, i) => (
                  <div key={i} className="relative">
                    ...
                    <motion.div className="glass-card rounded-xl p-5" whileHover={{ y: -2 }} transition={{ duration: 0.2 }}>
                      <div className="flex flex-wrap items-start justify-between gap-1 mb-1">
                        <h4 className="text-text-primary font-semibold text-[0.95rem]">{exp.title}</h4>
                        <span className="text-[11px] text-text-muted whitespace-nowrap">{exp.duration}</span>
                      </div>
            ...
            <motion.div variants={itemVariants} className="space-y-8">
              <div>
                <div className="flex items-center gap-2 mb-6">
                  ...
                  <h3 className="text-xs font-semibold text-text-muted uppercase tracking-widest">Education</h3>
                </div>
                <div className="space-y-4">
                  {education.map((edu, i) => (
                    <motion.div key={i} className="glass-card rounded-xl p-5" whileHover={{ y: -2 }} transition={{ duration: 0.2 }}>
                      <div className="flex flex-wrap items-start justify-between gap-1 mb-1">
                        <h4 className="text-text-primary font-semibold text-[0.95rem]">{edu.degree}</h4>
              ...
              <div>
                <div className="flex items-center gap-2 mb-5">
                  ...
                  <h3 className="text-xs font-semibold text-text-muted uppercase tracking-widest">Certifications</h3>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {certifications.map((cert, i) => (
                    <motion.div key={i} className="glass-card rounded-xl p-4 flex items-center gap-3" whileHover={{ y: -2 }} transition={{ duration: 0.2 }}>
          ...
          <motion.div variants={itemVariants}>
            ...
            <div className="flex items-center gap-2 mb-6">
              ...
              <h3 className="text-xs font-semibold text-text-muted uppercase tracking-widest">Key Highlights</h3>
            </div>
