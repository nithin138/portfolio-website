...
const Footer = () => {
  ...
  const contactItems = [
    { icon: <Mail size={15} />, label: 'Email', value: 'nnsudheer138@gmail.com', href: 'mailto:nnsudheer138@gmail.com' },
    { icon: <Github size={15} />, label: 'GitHub', value: 'github.com/nithin138', href: 'https://github.com/nithin138' },
    { icon: <Linkedin size={15} />, label: 'LinkedIn', value: 'linkedin.com/in/nithin138', href: 'https://www.linkedin.com/in/nithin138/' },
  ];
  ...
  return (
    <footer className="relative overflow-hidden" style={{ borderTop: '1px solid rgba(255,255,255,0.07)' }}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-14 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-8">

          <div className="flex flex-col gap-4">
            <div>
              <p className="text-lg font-bold text-text-primary tracking-tight leading-tight">Nithin Sudheer</p>
            ...
            <div className="flex items-center gap-2 mt-1">
              {socialLinks.map((s) => (
                <a
                  ...
                  onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(37,99,235,0.4)'; e.currentTarget.style.background = 'rgba(37,99,235,0.08)'; }}
          ...
          <div className="flex flex-col gap-4">
            <p className="text-xs font-semibold uppercase tracking-widest text-text-muted">Navigation</p>
            <div className="grid grid-cols-2 gap-x-6 gap-y-1">
              {[...navCol1, ...navCol2].map((link) => (
                <button
                  key={link.href} onClick={() => scrollToSection(link.href)}
              ...
              {contactItems.map((item) => (
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-sm text-text-secondary group-hover:text-text-primary transition-colors duration-200 truncate">{item.value}</p>
                  </div>
                  <ArrowUpRight size={13} className="ml-auto text-text-muted opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex-shrink-0" />
                </a>
              ))}
            </div>
          </div>
      ...
      <div style={{ borderTop: '1px solid rgba(255,255,255,0.07)' }}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row items-center justify-between gap-2">
          <p className="text-xs text-text-muted">&copy; {currentYear} Nithin Sudheer. All rights reserved.</p>
          <p className="text-xs text-text-muted">Built with React + Vite</p>
        </div>
      </div>
    </footer>
  );
